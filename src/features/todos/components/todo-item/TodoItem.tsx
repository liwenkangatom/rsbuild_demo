...
export const TodoItem = (props: TodoItemPropsType) => {
  ...
  return (
    <div className="  relative snap-start  flex flex-row w-full content-center shadow-md  border border-purple-400 rounded-lg p-2 bg-purple-100 text-gray-600   ">
      ...
      <Reduce onClick={() => !!onDelete && onDelete()} className='hover:scale-150 hover:translate-x-2 hover:-translate-y-2 active:scale-125 transition-transform fill-purple-900 bg-purple-100 shadow-md size-4 rounded-full right-0 top-0 absolute cursor-pointer' />
    </div>
  );
};
